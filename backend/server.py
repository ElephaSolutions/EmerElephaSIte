...
app = FastAPI(title="Elepha Solutions API")
...
@api_router.get("/")
async def root():
    return {"message": "Elepha Solutions API is running."}
...
async def submit_contact(payload: ContactInquiryCreate):
    ...
    try:
        ...
        logging.exception("Failed to store contact inquiry")
        raise HTTPException(status_code=500, detail="Unable to submit inquiry right now.") from e
    return inquiry
